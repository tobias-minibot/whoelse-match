...
3. **Invite copy must be true.** Production `WHOELSE_SEED` stays unset. The pool is empty until real humans publish. Seed cards, if any preview is on, stay labeled synthetic/AI — never fake humans as real. **Invoke HTTP is a stub** (structured “I would do X”). Receipts and reputation persist; live execution does not.
